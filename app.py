...
# Load the data
df = pd.read_csv("rake-wine-full.csv")
...
df["vivino_price"] = df["vivino_price"].apply(lambda x: float(x) if x != "N/A" else 0)
...
size = st.sidebar.multiselect("Size", df["size"].unique(), default=df["size"].unique())
